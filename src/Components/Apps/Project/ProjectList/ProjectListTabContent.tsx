    ...
    <Card>
      <CardBody className='pb-0'>
        <TabContent activeTab={activeTab}>
          ...
          <TabPane tabId='2'>
            <Row>{createdFormData.map((item, i) => (item.badge === "Doing" ? <ProjectCommon item={item} key={i} /> : " "))}</Row>
          </TabPane>
          <TabPane tabId='3'>
            <Row>{createdFormData.map((item, i) => (item.badge === "Done" ? <ProjectCommon item={item} key={i} /> : " "))}</Row>
          </TabPane>
        </TabContent>
      </CardBody>
    </Card>
