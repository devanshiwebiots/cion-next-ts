...
import { paymentMethodData } from "@/Data/Form/FormControl/FormControlData";
          ...
          <div className='payment-wrapper' key={index}>
            <div className='payment-first'>
              <FormGroup check className='radio radio-primary'>
                ...
                <Label className='form-check-label mb-0' htmlFor={`paymentMethod-${index}`}>
                  {data.name}
                </Label>
              </FormGroup>
            </div>
            <div className='payment-second'>
              <Image className='img-fluid' width={50} height={34} src={`${ImagePath}/ecommerce/${data.imagePath}.png`} alt='card' />
            </div>
          </div>
        ))}
      </div>
